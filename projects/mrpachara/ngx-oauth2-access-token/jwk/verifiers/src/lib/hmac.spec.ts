...
describe('Verifiers', () => {
  ...
  it('should return undefined if no JWK is an HMAC key', async () => {
    const nonHmacJwk = { kty: 'RSA' } as unknown as Jwk;

    const result = await hmac(mockJwsInfo, [nonHmacJwk]);
  ...
  it('should verify signature successfully', async () => {
    ...
    const result = await hmac(mockJwsInfo, [mockJwk]);
