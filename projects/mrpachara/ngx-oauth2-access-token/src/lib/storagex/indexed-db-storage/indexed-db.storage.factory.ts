...
class IndexedDbStorage implements KeyValuePairsStorage {
  ...
  async loadItem<T = unknown>(key: string): Promise<DeepReadonly<T | null>> {
    ...
    return deepFreeze(
      (await promisifyRequest<T | undefined>(storage.get(storageKey))) ?? null,
    );
  }
  ...
  async storeItem<T = unknown>(
    ...
    await promisifyRequest(storage.put(value, storageKey));

    this.emitKey(storageKey);

    return deepFreeze<T>(await promisifyRequest<T>(storage.get(storageKey)));
  }
  ...
  async removeItem<T = unknown>(key: string): Promise<DeepReadonly<T | null>> {
    ...
    const value = deepFreeze(
      (await promisifyRequest<T>(storage.get(storageKey))) ?? null,
    );
    await promisifyRequest(storage.delete(storageKey));
  ...
  async keys(): Promise<string[]> {
    ...
    return (await promisifyRequest(storage.getAllKeys()))
      .map((rawKey) => `${rawKey}`)
